...
interface ScaneoActivosProps {
    activosFacturaResponsable: VwMovimientosAF[];
    infoResponsable?: {
        responsable_nombre: string;
    } | null;
    infoFactura?: FacturasAF | null;
}

export const MyDocument = ({ activosFacturaResponsable, infoResponsable, infoFactura }: ScaneoActivosProps) => (
    <Document>
        <Page size="LETTER" style={styles.page}>
            <View>
                ...
                <View style={styles.section}>
                    <Text style={styles.subtitle}>Activos Entregados ({activosFacturaResponsable.length})</Text>
                    <View style={styles.table}>
                        ...
                        {activosFacturaResponsable.length === 0 ? (
                            <View style={styles.tableRow}>
                                <Text style={styles.tableCell}>Sin registros</Text>
                            </View>
                        ) : (
                            activosFacturaResponsable.map((activo, idx) => (
            ...
            <View style={styles.signatureSection} fixed>
                ...
                <View style={{ alignItems: 'center', flex: 1 }}>
                    <View style={styles.signatureBox}></View>
                    <Text style={styles.signatureLabel}>{infoResponsable?.responsable_nombre || 'Sin información de responsable'}</Text>
                </View>
            </View>
        </Page>
    </Document>
