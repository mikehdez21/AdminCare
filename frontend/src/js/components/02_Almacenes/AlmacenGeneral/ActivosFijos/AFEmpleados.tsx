...
const AFEmpleados: React.FC<AFEmpleadosProps> = ({ empleadoSeleccionadoId, onSelectEmpleado }) => {
  ...
  // Filtros de búsqueda (ordenados alfabéticamente)
  const empleadosDisponiblesFiltrados = empleados
    .filter(empleado => {
  ...
  const handleSeleccionarEmpleado = async (empleado: Empleados) => {
    ...
    navigate(`/almacen-general/activos/empleado/${empleado.id_empleado}`);
