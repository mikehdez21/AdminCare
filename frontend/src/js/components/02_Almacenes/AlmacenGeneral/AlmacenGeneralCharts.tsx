...
import { getActivosFijos, getActivosFijosSinFactura } from '@/store/almacengeneral/Activos/activosActions';
...
const AlmacenGeneralCharts: React.FC = () => {
  ...
  const [activosSinFactura, setActivosSinFactura] = React.useState<ActivosFijos[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (activosSinFactura.length === 0) {
        tasks.push((async () => {
          const resSinFactura = await dispatch(getActivosFijosSinFactura()).unwrap();
          if (resSinFactura.success && resSinFactura.activosFijos) {
            setActivosSinFactura(resSinFactura.activosFijos);
          }
        })());
      }
  ...
  const activosPorClasificacion = useMemo(() => {
    ...
    if (activosMovimientos.length > 0) {
      activosMovimientos.forEach((a) => {
        const key = a.clasificacion || 'ACTIVOS MENORES';
      ...
      activos.forEach((a) => {
        const key = a.id_clasificacion ? (clasifMap.get(a.id_clasificacion) || 'ACTIVOS MENORES') : 'ACTIVOS MENORES';
        map.set(key, (map.get(key) || 0) + 1);
      });
    }
  ...
  return (
    <div className="agc-wrapper">
      <div className="agc-kpi-grid">
        ...
        <div className="agc-kpi-card">
          <h4> Bajas de Activos</h4>
          <p>{totalActivosInactivos}</p>
        </div>
        <div className="agc-kpi-card">
          <h4> Activos No Propios</h4>
          <p>{totalActivosNoPropios}</p>
        </div>
        <div className="agc-kpi-card">
          <h4> Activos Sin Factura</h4>
          <p>{activosSinFactura.length}</p>
        </div>
      </div>
