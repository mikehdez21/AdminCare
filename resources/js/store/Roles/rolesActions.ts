  ...
  async (rolEditado: Roles) => {
    try {
      ...
      const response = await axios.put(
        `http://pruebas.hssadmincare.web/api/HSS1/almacenGeneral/admin/roles/${rolEditado.id}`,
        rolEditado,
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  async (rolEliminado: Roles) => {
    try {
      ...
      const response = await axios.delete(
        `http://pruebas.hssadmincare.web/api/HSS1/almacenGeneral/admin/roles/${rolEliminado.id}`,
        {
          headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': csrfToken || '',
