...
// Styles
import '@styles/02_Almacenes/AlmacenGeneral/clasificacionesAFControl.css';
...
const AlmacenGeneral_ControlClasificacion: React.FC = () => {
  ...
  const [clasificacionToEdit_Delete, setClasificacionToEdit_Delete] = useState<Clasificaciones | null>(null); // clasificacion seleccionado para editar_eliminar
  ...
  const openModalEditClasificacion = (clasificacion: Clasificaciones) => {
  ...
  const openAlertDeleteClasificacion = (clasificacion: Clasificaciones) => {
    setClasificacionToEdit_Delete(clasificacion); // Establecer el clasificacion seleccionado
    ...
    const cargarClasificaciones = async () => {
      try {
        ...
        if(resultAction.success){
          dispatch(setClasificacion(resultAction.clasificacion!)); // Establece el clasificacion en el estado
  ...
  const clasificaciones = useSelector((state: RootState) => state.clasificacion.clasificaciones);
  ...
  const clasificacionesFiltradas = Array.isArray(clasificaciones) 
    ? clasificaciones
      .filter(clasificacion =>
        clasificacion.descripcion_clasificacionaf.toLowerCase().includes(busqueda.toLowerCase()) ||
              ...
              <tbody>
                {clasificacionesPaginaActual.map(clasificacion => (
                  <tr key={clasificacion.id_clasificacion}>
                    <td id='td_ID'>{clasificacion.id_clasificacion}</td>
                    <td id='td_Clasificacion'>{clasificacion.descripcion_clasificacionaf}</td>
